...
exports.createGroup = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!groupnameRegex.test(group_name)) {
    res.status(400).json({
      success: false,
      message:
        "Error: Groupname can only be alphanumeric and contain no spaces",
    });
    return;
  }
  ...
  const [row, fields] = await connection
    .promise()
    .query("SELECT * FROM usergroups WHERE group_name = ?", [group_name]);
  if (row.length !== 0) {
  ...
  const result = await connection
    .promise()
    .execute("INSERT INTO usergroups (group_name) VALUES (?)", [group_name]);
